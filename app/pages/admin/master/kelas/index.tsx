...
import type { KelasResponse } from "./type";
...
const index = () => {
  ...
  const { data, error, isLoading } = useSWR<KelasResponse>(URL, fetcher);
  ...
  const deleteData = async (id: number) => {
    dialog.setOnConfirm(async () => {
      const url = `${import.meta.env.VITE_BASE_URL}/api/kelas/${id}`;
  ...
  return (
    <div className="flex flex-1 flex-col space-y-2 p-4 px-5">
      ...
      <div className="flex items-start justify-between">
        <Heading
          title="Kelas"
          description="Manage kelas (Server side table functionalities.)"
        />
        <Button
          onClick={() => {
            store.setData(null);
            ...
            <TableBody>
              {data?.results.data.map((kelas, index) => (
                <TableRow key={kelas.id}>
                  ...
                  <TableCell>{kelas.code}</TableCell>
                  <TableCell>{kelas.name}</TableCell>
                  <TableCell>
                    <Badge>{kelas.level.toUpperCase()}</Badge>
                  </TableCell>
