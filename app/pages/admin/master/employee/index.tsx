...
const index = () => {
  ...
  const { data, error, isLoading } = useSWR<EmployeeResponse>(URL, fetcher);
  ...
  const deleteData = async (id: number) => {
    dialog.setOnConfirm(async () => {
      const url = `${import.meta.env.VITE_BASE_URL}/api/employee/${id}`;
  ...
  return (
    <div className="flex flex-1 flex-col space-y-2 p-4 px-5">
      ...
      <div className="flex items-start justify-between">
        <Heading
          title="Employee"
          description="Manage employee (Server side table functionalities.)"
        />
        <Button
          onClick={() => {
            store.setData(null);
            ...
            <TableBody>
              {data?.results.data.map((item, index) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell>
                    <Badge className="capitalize">{item.position}</Badge>
                  </TableCell>
                  ...
                  <TableCell>{item.user?.birth_place || "-"}</TableCell>
                  <TableCell>{item.user?.birth_date || "-"}</TableCell>
                  <TableCell>{item.user?.phone_number || "-"}</TableCell>
                  <TableCell>{item.user?.nationality || "-"}</TableCell>
