...
import type { TahunAkademikResponse } from "./type";
...
const index = () => {
  ...
  const { data, error, isLoading } = useSWR<TahunAkademikResponse>(
    URL,
    fetcher
  );
  ...
  const deleteData = async (id: number) => {
    dialog.setOnConfirm(async () => {
      const url = `${import.meta.env.VITE_BASE_URL}/api/tahun-akademik/${id}`;
    ...
    <div className="flex flex-1 flex-col space-y-2 p-4 px-5">
      ...
      <div className="flex items-start justify-between">
        <Heading
          ...
          description="Manage tahun akademik (Server side table functionalities.)"
        />
        <Button
          onClick={() => {
            store.setData(null);
